<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button v-permission="['admin:life:list']" class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">查找</el-button>
    </div>
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" label="操作" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="scope.row.pushStatus === 1" v-permission="['admin:life:push']" type="primary" size="mini"
                     @click="handlePush(scope.row,2)">推送
          </el-button>
          <el-button v-if="scope.row.pushStatus === 1" v-permission="['admin:life:push']" type="danger" size="mini"
                     @click="handlePush(scope.row,3)">回绝
          </el-button>
          <el-button v-permission="['admin:life:delete']" type="danger" size="mini"
  ...
  export default {
    ...
    methods: {
      ...
      handlePush(row, status) {
        pushLife(row.id, status).then(res => {
            this.$notify.success({
              title: '成功',
