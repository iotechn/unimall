<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.number" clearable class="filter-item" style="width: 200px;" placeholder="请输入群号搜索"/>
      <el-select v-model="listQuery.type" style="width: 200px" class="filter-item" placeholder="请选择查找类型">
        <el-option v-for="(key, value) in typeMap" :key="key" :label="key" :value="value"/>
      </el-select>
      <el-button v-permission="['admin:qq:list']" class="filter-item" type="primary" icon="el-icon-search"
                 @click="handleFilter">查找
      </el-button>
      <el-button v-permission="['admin:qq:create']" class="filter-item" type="primary" icon="el-icon-create"
                 @click="showCreate">添加
      </el-button>
    </div>
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['admin:qq:delete']"  type="primary"
                     size="mini" @click="handleDel(scope.row)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog title="添加" :visible.sync="createDialogVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" status-icon label-position="left" label-width="100px"
...
<script>
  import {list, create, update, del} from '@/api/qq'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          title: [
            {required: true, message: '群标题不能为空', trigger: 'blur'}
          ],
          type: [
            {required: true, message: '群类型不能为空', trigger: 'blur'}
          ],
          number: [
            {required: true, message: '群号码不能为空', trigger: 'blur'}
          ]
        },
      }
    },
