<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-select v-model="listQuery.status" style="width: 200px" class="filter-item" placeholder="请选择申请表状态">
        <el-option v-for="(key, value) in statusMap" :key="key" :label="key" :value="value"/>
      </el-select>
      <el-button v-permission="['admin:apply:list']" class="filter-item" type="primary" icon="el-icon-search"
                 @click="handleFilter">查找
      </el-button>
      <el-button v-permission="['admin:apply:apply']" class="filter-item" type="primary" icon="el-icon-search"
                 @click="showApply">申请
      </el-button>
    </div>
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-permission="['admin:apply:handle']" v-if="scope.row.status==0" type="primary"
                     size="mini" @click="handleDeal(scope.row, 1)">通过
          </el-button>
          <el-button v-permission="['admin:apply:handle']" v-if="scope.row.status==0" type="primary"
                     size="mini" @click="handleDeal(scope.row, 2)">回绝
          </el-button>
    ...
    <el-dialog title="申请" :visible.sync="applyDialogVisible">
      <el-form ref="dataForm" :rules="rules" :model="applyDetail" status-icon label-position="left" label-width="100px"
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        listApply(this.listQuery).then(response => {
      ...
      handleDeal(row,status) {
        this.$confirm('您确定要标记 ' + row.realname + ' 的请求为' + (status == 1? '已通过' : '已回绝') + '吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          applydeal(row.id, status).then(response => {
            this.$notify.success({
              title: '成功',
              message: '确认处理成功'
            })
