<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.name" clearable class="filter-item" style="width: 200px;" placeholder="请输入昵称"/>
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" min-width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-permission="['admin:user:status']" type="primary" size="mini" @click="handleStatus(scope.row)">{{scope.row.status === 0? '激活' : '冻结'}}</el-button>
        </template>
      </el-table-column>
    </el-table>
