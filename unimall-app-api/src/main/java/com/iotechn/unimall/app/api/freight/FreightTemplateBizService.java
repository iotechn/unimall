...
public class FreightTemplateBizService {
    ...
    @Autowired
    FreightTemplateCarriageMapper freightTemplateCarriageMapper;
    @Autowired
    ...
    public Integer getFreightMoney(OrderRequestDTO orderRequestDTO){
        ...
        Integer discount = orderRequestDTO.getCoupon().getDiscount();
        for(Long key : freightNum.keySet()){
            totalMoney = totalMoney + getMoney(freightDTO.get(key),freightNum.get(key),addressDO.getProvince(),freightMoney.get(key)-discount);
        }
        return totalMoney;
    }

    //根据传入的运费模板信息\使用同一模板商品数量\快递省份地址\使用同一模板商品总价减使用的优惠卷，计算这类商品在该订单中的运费
    public Integer getMoney(FreightTemplateDTO freightTemplateDTO,Integer num,String province,Integer moneySubDiscount){
        ...
        if(num % freightTemplateDO.getDefaultContinueNum() != 0){
            open = open + freightTemplateDO.getDefaultContinueNum() * ((num / freightTemplateDO.getDefaultContinueNum())+1);
        }else {
