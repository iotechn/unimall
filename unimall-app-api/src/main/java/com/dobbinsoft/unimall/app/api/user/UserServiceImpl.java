package com.dobbinsoft.unimall.app.api.user;
...
import com.dobbinsoft.unimall.data.util.PKCS7Encoder;
...
public class UserServiceImpl extends BaseService<UserDTO, AdminDTO> implements UserService {
    ...
    public String bindPhone(String phone, String verifyCode, Long userId) throws ServiceException {
        //1.校验验证码
        checkVerifyCode(phone, verifyCode);
        cacheComponent.del(CacheConst.USER_VERIFY_CODE_PREFIX + phone);
        // 开始业务
        ...
        if (userDO.getStatus().intValue() == UserStatusType.WAIT_PHONE.getCode()) {
            //2. 绑定 或 合成逻辑
            if (tryMergeUser(phone, userDO, existUserDO)) return "ok";
     ...
     * @return
     */
    private boolean tryMergeUser(String phone, UserDO currentUserDO, UserDO existUserDO) {
        UserDO updateUserDO = new UserDO();
        if (existUserDO != null) {
            // 2.1. 合成逻辑
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public String getWxPhone(WxLoginRequestDTO raw, String encryptedData, String iv, Long userId) throws ServiceException {
        try {
            ...
            if (errcode == null || errcode == 0) {
                ...
                UserDO userDO = userMapper.selectById(userId);
                if (userDO.getStatus().intValue() == UserStatusType.WAIT_PHONE.getCode()) {
                    this.tryMergeUser(purePhoneNumber, userDO, existUserDO);
                    // 更新用户Session信息
                    UserDTO user = sessionUtil.getUser();
    ...
    @Override
    public String syncUserInfo(String nickname, String avatarUrl, String province, String city, String county, Integer gender, LocalDateTime birthday, Long userId) throws ServiceException {
        UserDO updateUserDO = new UserDO();
        updateUserDO.setId(userId);
