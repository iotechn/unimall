    ...
    <view class="wrapper">
      ...
      <view v-if="loginType === 'phone'" class="input-content">
        <view class="input-item">
          ...
          <input :value="phone" type="number" placeholder="请输入手机号码" maxlength="11" data-key="phone" @input="inputChange">
        </view>
        <view class="input-item">
          <text class="tit">
            密码
      ...
      <button v-if="!loginType" :disabled="logining" class="confirm-btn" @click="chooseLoginType('wechat')">
        微信快速登录（推荐）
      </button>
      <!-- #endif -->
      <!-- #ifdef MP-ALIPAY -->
      <button v-if="!loginType" :disabled="logining" class="confirm-btn" @click="chooseLoginType('alipay')">
        支付宝快速登录（推荐）
      </button>
      <!-- #endif -->
      <!-- #ifdef APP-PLUS -->
      <button v-if="!loginType" :disabled="logining" class="confirm-btn" @click="chooseLoginType('phone')">
        手机注册登录
      </button>
      <!-- #endif -->
      <!-- #ifdef MP-WEIXIN -->
      ...
      <!-- #endif -->
      <!-- #ifdef APP-PLUS -->
      <button v-if="loginType === 'wechat'" :disabled="logining" class="confirm-btn" @click="wechatLogin">
        微信授权登录
      </button>
      <!-- #endif -->
      <!-- #ifdef H5 -->
      <button v-if="loginType === 'wechat'" :disabled="logining" class="confirm-btn" @click="wechatH5Login">
        微信授权登录
      </button>
      <!-- #endif -->
      <!-- #ifdef MP-ALIPAY -->
      <button v-if="loginType === 'alipay'" :disabled="logining" class="confirm-btn" open-type="getAuthorize" scope="userInfo" @click="alipayLogin()">
        支付宝授权登录
      </button>
      <!-- #endif -->
      <!-- #ifdef APP-PLUS -->
      <button v-if="loginType === 'alipay'" :disabled="logining" class="confirm-btn" @click="alipayLogin">
        支付宝授权登录
      </button>
      <!-- #endif -->
      <button v-if="loginType === 'phone'" :disabled="logining" class="confirm-btn" @click="toLogin">
        登录
      </button>
