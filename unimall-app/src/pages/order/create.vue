  ...
  <view>
    ...
    <navigator url="/pages/address/list?source=1" class="address-section">
      <view class="order-content">
        ...
        <view class="cen">
          ...
          <text class="address">
            {{ addressData.province }} {{ addressData.city }} {{ addressData.county }} {{ addressData.address }}
          </text>
        </view>
      ...
      <view v-for="(item, index) in orderReqeust.skuList" :key="index" class="g-item">
        ...
        <view class="right">
          ...
          <view class="price-box">
            <text class="price">
              <text
                v-if="(isVip ? item.vipPrice : item.price) < item.originalPrice"
                style="text-decoration:line-through; font-size: 25upx; color: #666666;"
              >
                ￥{{ item.originalPrice / 100.0 }}
              </text>￥{{ (isVip ? item.vipPrice : item.price) / 100.0 }}
            </text>
            <text class="number">
              x {{ item.num }}
            </text>
    ...
    <view class="yt-list">
      <view class="yt-list-cell b-b" @click="toggleMask('show')">
        ...
        <text class="cell-more wanjia wanjia-gengduo-d" />
      </view>
    </view>
    <!-- 金额明细 -->
    <view class="yt-list">
      <view class="yt-list-cell b-b">
        ...
        <text class="cell-tip">
          ￥{{ orderReqeust.totalOriginalPrice / 100.0 }}
        </text>
      </view>
      <view v-if="orderReqeust.totalOriginalPrice - orderReqeust.totalPrice > 0" class="yt-list-cell b-b">
        ...
        <text class="cell-tip red">
          -￥{{ (orderReqeust.totalOriginalPrice - orderReqeust.totalPrice) / 100.0 }}
        </text>
      </view>
      <view v-if="orderReqeust.coupon" class="yt-list-cell b-b">
        ...
        <text class="cell-tip red">
          -￥{{ orderReqeust.coupon.discount / 100.0 }}
        </text>
      </view>
      ...
      <view v-if="orderReqeust.freightPrice > 0" class="yt-list-cell b-b">
        ...
        <text class="cell-tip">
          ¥ {{ orderReqeust.freightPrice / 100.0 }}
        </text>
      </view>
      <view class="yt-list-cell desc-cell">
        ...
        <input v-model="orderReqeust.mono" class="desc" type="text" placeholder="请填写备注信息" placeholder-class="placeholder">
      </view>
    </view>
    ...
    <view class="footer">
      <view class="price-content">
        ...
        <text class="price">
          {{ (orderReqeust.totalPrice - (orderReqeust.coupon?orderReqeust.coupon.discount:0) + orderReqeust.freightPrice) / 100.0 }}
        </text>
      </view>
    ...
    <!-- 优惠券面板 -->
    <view :class="maskState===0 ? 'none' : maskState===1 ? 'show' : ''" class="mask" @click="toggleMask">
      <view class="mask-content" @click.stop.prevent="stopPrevent">
        <!-- 优惠券页面，仿mt -->
        <view v-for="(item,index) in couponList" v-show="(!item.categoryId && orderReqeust.totalPrice >= item.min) || (item.categoryId && skuCategoryPriceMap[item.categoryId] && skuCategoryPriceMap[item.categoryId] > item.min)" :key="index" class="coupon-item" @click="selectCoupon(item)">
          <view class="con">
            <view class="left">
              <text class="title">
                {{ item.title }}
