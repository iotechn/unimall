  ...
  <view>
    <view class="eva-section">
      ...
      <view v-for="(item,index) in page.items" :key="index" class="eva-box">
        <image :src="item.userAvatarUrl ? item.userAvatarUrl : '/static/missing-face.png'" class="portrait" mode="aspectFill" />
        <view class="right">
          <text class="name">
          ...
          <view v-if="item.imgList && item.imgList.length > 0" class="imgs">
            <image v-for="(imgItem, imgIndex) in item.imgList" :key="imgIndex" :src="imgItem + style(200)" class="ig" @click="previewImg(item.imgList, imgIndex)" />
          </view>
          <view class="bot">
            <text class="attr">
              购买类型：{{ item.skuTitle }}
